...
def make_wh2_example() -> dict:
    """Creates a random input example for the wh2 policy."""
    return {
        "state": np.ones((8,)),
        "images": {
...
class WH2Inputs(transforms.DataTransformFn):

    """Inputs for the WH2 policy.
...
@dataclasses.dataclass(frozen=True)
class WH2Outputs(transforms.DataTransformFn):
    def __call__(self, data: dict) -> dict:
